...
Bash function `loop` runs scripts from `<git-root>/loop/loop.sh` with optional tmux support:

```bash
loop                    # Run loop script directly
loop --tmux             # Run in tmux session (named after current directory)
loop --tmux arg1 arg2   # Pass arguments to script in tmux
```

With `--tmux`: creates new session or attaches to existing one with matching name.
