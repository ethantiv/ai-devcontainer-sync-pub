...
- **UTF-8 locale w Docker**: debian:bookworm-slim nie ma domyślnie wygenerowanych locale. Wymagane: `apt install locales`, `sed -i '/en_US.UTF-8/s/^# //g' /etc/locale.gen && locale-gen`, oraz `ENV LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8`. Bez tego polskie znaki w tmux wyświetlają się jako `_`.
