...
npm install --prefix src && npm test --prefix src  # Run JS tests (116 tests, requires install)
...
- **Config parser**: `src/lib/config-parser.js` reads `env-config.yaml`, merges defaults with environment overrides, interpolates `${VAR}` from env. Exports: `loadConfig`, `mergeConfig`, `interpolateVars`, `validateConfig`, `flattenSection`. CLI: `node config-parser.js --config <path> --env <env> --section <name>` (scalars→`KEY=value`, objects→JSON) or `--all` (full JSON). YAML configs: `.devcontainer/configuration/env-config.yaml` (real data, private) and `env-config.example.yaml` (template, public). Migration from `skills-plugins.txt` in progress.
