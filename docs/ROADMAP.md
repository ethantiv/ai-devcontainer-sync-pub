...
The ConversationHandler has 9 states with branching transitions across 5 handler modules. Add a Mermaid state diagram to `TELEGRAM_COMMANDS.md` showing the full flow: SELECT_PROJECT to PROJECT_MENU to each sub-flow (task, brainstorm, clone, create, worktree). This helps new contributors understand the bot's conversation logic without reading all handler code.
